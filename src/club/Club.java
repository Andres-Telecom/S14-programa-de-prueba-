...
public class Club
{
    ...
    public void afiliarSocio( String pCedula, String pNombre, Tipo pTipo ) throws Exception
    {
        ...
        if(s != null)
        {
            throw new   Exception("El socio con la cedula "+ pCedula + " ya existe.");
        }

        // Revisar que no se haya alcanzado el l�mite de subscripciones VIP
        if( pTipo == Tipo.VIP && contarSociosVIP( ) == MAXIMO_VIP )
        {
            throw  new Exception("El club en el momento no acepta mas socios VIP" );
    ...
    public void agregarAutorizadoSocio( String pCedulaSocio, String pNombreAutorizado ) throws Exception
    {
        Socio s = buscarSocio( pCedulaSocio );
        if (s == null) {
            throw new Exception("El socio con la cedula " + pCedulaSocio + " no existe.");
        }
    ...
    public double darTotalConsumo(String pCedula) throws Exception
    {
        Socio s = buscarSocio( pCedula );
        if (s == null) {
            throw new Exception("El socio con la cedula " + pCedula + " no existe.");
        }
    ...
    public boolean sePuedeEliminarSocio( String pCedula ) throws Exception
    {
        ...
        // Caso 1: Que no exista un socio con la cédula recibida
        if( s == null )
        {
            throw new Exception("No existe un socio con la cédula especificada.");
        }

        // Caso 2: Que el socio sea de tipo VIP
        if( s.darTipo() == Tipo.VIP )
        {
            throw new Exception("No se pueden eliminar socios de tipo VIP.");
        }

        // Caso 3: Que el socio tenga facturas pendientes de pago
        if( s.darFacturas() != null && !s.darFacturas().isEmpty() )
        {
            throw new Exception("No se pueden eliminar socios con facturas pendientes de pago.");
        }

        // Caso 4: Que el socio tenga más de un autorizado
        if( s.darAutorizados() != null && s.darAutorizados().size() > 1 )
        {
            throw new Exception("No se puede eliminar un socio con más de un autorizado.");
        }
